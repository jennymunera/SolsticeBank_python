...
def makegraphs_Olimpica(df):
        
        df_Olimpica = df[df['device']== 'bc37']
        df_Olimpica.reset_index(inplace=True)
        df_Olimpica_tension = df_Olimpica[df_Olimpica['variable'].isin(['front-tension-1','front-tension-2','front-tension-3'])]
        df_Olimpica = df_Olimpica[df_Olimpica['variable'].isin(['front-consumo-activa','aa-consumo-activa','ilu-consumo-activa'])]

        df_Olimpica_front = df_Olimpica[df_Olimpica['variable'] == 'front-consumo-activa']
        df_Olimpica_front['datetime'] = pd.to_datetime(df_Olimpica_front['datetime'	])
        df_Olimpica_front['hour'] = df_Olimpica_front['datetime'].dt.hour
        ...
        consumo_front = np.sum(df_Olimpica[df_Olimpica['variable'] == 'front-consumo-activa']['value'])
        potencia_maxima = np.max(df_Olimpica[df_Olimpica['variable'] == 'front-consumo-activa']['value'])
        maxima_demanda = np.max(df_Olimpica[df_Olimpica['variable'] == 'aa-consumo-activa']['value'])/12
        ...
        with graph3:
            fig_hist = px.histogram(df_Olimpica_front, x='value', nbins=12, title="Histograma de potencia activa (kW)")
            st.plotly_chart(fig_hist,use_container_width=True)

        with graph4:
            fig_pie = px.pie(df_Olimpica, values='value', names='variable', title='Consumo total por dispositivo')
